...
(function (root, factory) {
  ...
  /** Bombers / INFOCA / INFOCAM / Aragón open-incident window. */
  const REGIONAL_LOOKBACK_DAYS = 14;
  /** INFOCAM controlled mega-fires can stay open for weeks after detección. */
  const INFOCAM_LOOKBACK_DAYS = 60;
  ...
  const INFOCAM_PORTAL_URL = "https://fidias.castillalamancha.es/";
  const ARAGON_IDE_URL = "https://idearagon.aragon.es/";
  ...
  function normalizeInfocam(feature) {
    const p = feature.properties || feature.attributes || {};
    const coords = featureCoords(feature) || {};
    const lat =
      coords.lat != null ? coords.lat : Number(propGet(p, ["Latitud", "latitud"]));
    const lng =
      coords.lng != null ? coords.lng : Number(propGet(p, ["Longitud", "longitud"]));
    const status = String(propGet(p, ["Estado", "estado"]) || "ACTIVO").trim();
    const municipality = shortMunicipality(propGet(p, ["Municipio", "municipio"]));
    const province = String(propGet(p, ["Provincia", "provincia"]) || "C-LM").trim();
    const detectMs = epochMs(propGet(p, ["Detección", "Deteccion", "deteccion"]));
    const controlMs = epochMs(propGet(p, ["Control", "control"]));
    const parteMs = controlMs || detectMs;
    const code = propGet(p, ["CódigoIF", "CodigoIF", "codigoIF", "FID", "OBJECTID"]);
    const ha = Number(propGet(p, ["SupTotal", "supTotal"]));
    const surface = Number.isFinite(ha) && ha > 0 ? `${ha} ha` : "—";
    const cause =
      propGet(p, ["Tipo_Siniestro_Descripción", "Tipo_Siniestro_Descripcion", "Tipo_Siniestro"]) ||
      "Incendio forestal";
    return {
      id: `clm:${code}`,
      country: "ES",
      source: SOURCE.INFOCAM,
      municipality,
      province,
      status,
      statusClass: statusClass(status),
      ...
      terrain: 0,
      aerial: 0,
      started: formatEsDateTime(detectMs),
      parteAt: formatEsDateTime(parteMs),
      parteMs,
      rawOrden: "",
      lat: Number.isFinite(lat) ? lat : null,
      lng: Number.isFinite(lng) ? lng : null,
      locationLine: [municipality, province].filter(Boolean).join(", "),
      detailUrl: INFOCAM_PORTAL_URL,
    };
  }

  /**
   * INFOCAM C-LM GeoJSON (Worker `/infocam`) → open forestals (not extinguished).
   * @param {object} payload FeatureCollection
   */
  function filterInfocamRows(payload, nowMs) {
    const now = nowMs == null ? Date.now() : nowMs;
    const cutoff = now - INFOCAM_LOOKBACK_DAYS * 24 * 36e5;
    const open = new Set(["activo", "controlado", "estabilizado", "declarado"]);
    return geojsonFeatures(payload)
      .filter((feature) => {
        const p = feature.properties || feature.attributes || {};
        if (String(propGet(p, ["Siniestro", "siniestro"]) || "").toUpperCase() !== "FORESTAL") {
          return false;
        }
        const falsa = String(propGet(p, ["FalsaAlarma", "falsaAlarma"]) || "NO")
          .trim()
          .toUpperCase();
        if (falsa === "SI" || falsa === "SÍ" || falsa === "YES" || falsa === "TRUE") return false;
        if (epochMs(propGet(p, ["Extinción", "Extincion", "extincion"]))) return false;
        const st = normalizeStatusKey(propGet(p, ["Estado", "estado"]) || "");
        if (st === "extinguido" || st === "extinto") return false;
        if (st && !open.has(st)) return false;
        const coords = featureCoords(feature);
        const lat = coords ? coords.lat : Number(propGet(p, ["Latitud", "latitud"]));
        const lng = coords ? coords.lng : Number(propGet(p, ["Longitud", "longitud"]));
        if (!Number.isFinite(lat) || !Number.isFinite(lng)) return false;
        if (!pointInSpain(lat, lng)) return false;
        // Rough C-LM envelope (drop distant corrupt points).
        if (lat < 37.8 || lat > 41.6 || lng < -5.6 || lng > -0.6) return false;
        const detectMs = epochMs(propGet(p, ["Detección", "Deteccion", "deteccion"]));
        const controlMs = epochMs(propGet(p, ["Control", "control"]));
        const parteMs = controlMs || detectMs;
        if (parteMs && parteMs < cutoff) return false;
      ...
      .map(normalizeInfocam)
      .sort(compareFires);
  }

  function normalizeAragon(feature) {
    const p = feature.properties || feature.attributes || {};
    const coords = featureCoords(feature) || {};
    const oid = p.id != null ? p.id : p.objectid != null ? p.objectid : feature.id;
    return {
      id: `ara:${oid}`,
      country: "ES",
      source: SOURCE.ARAGON,
      municipality: `Incendio #${oid}`,
      province: "Aragón",
      status: "ACTIVO",
      statusClass: "activo",
      level: "—",
      cause: "Incendio forestal",
      surface: "—",
      resourcesText: "",
      man: 0,
      terrain: 0,
      ...
      lat: coords.lat != null ? coords.lat : null,
      lng: coords.lng != null ? coords.lng : null,
      locationLine: `Aragón · #${oid}`,
      detailUrl: ARAGON_IDE_URL,
    };
  }

  /**
   * Aragón CartoFor WFS `INCENDIOS_ACTIVOS` (Worker `/aragon`).
   * Layer is already active-only; metadata is sparse (id + point).
   * @param {object} payload FeatureCollection
   */
  function filterAragonRows(payload) {
    return geojsonFeatures(payload)
      .filter((feature) => {
        const p = feature.properties || feature.attributes || {};
        if (p.esactivo != null && Number(p.esactivo) === 0) return false;
        const coords = featureCoords(feature);
        if (!coords) return false;
        if (!pointInSpain(coords.lat, coords.lng)) return false;
        if (coords.lat < 39.7 || coords.lat > 43.0 || coords.lng < -2.4 || coords.lng > 1.0) {
          return false;
        }
        return true;
      })
